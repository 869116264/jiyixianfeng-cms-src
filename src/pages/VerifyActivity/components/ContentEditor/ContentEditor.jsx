...
const statusList = ['保存草稿状态', '报名开始', '报名结束', '活动开展准备中', '进行中', '已结束'];
...
export default class ContentEditor extends Component {
  ...
  agree = () => {
    ...
    this.postForm.validateAll((errors, values) => {
      ...
      fmdata.set('activity_id', data.id);
      axios({
        method: 'POST',
        url: 'https://igulu.net/api/passActivity',
        data: fmdata,
        config: { headers: { 'Content-Type': 'multipart/form-data' } },
      })
        .then(function (response) {
  ...
  disagree = () => {
    ...
    this.postForm.validateAll((errors, values) => {
      ...
      axios({
        method: 'POST',
        url: 'https://igulu.net/api/denyActivity',
        data: fmdata,
        config: { headers: { 'Content-Type': 'multipart/form-data' } },
      })
        .then(function (response) {
