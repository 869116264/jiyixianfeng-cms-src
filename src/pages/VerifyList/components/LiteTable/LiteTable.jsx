  ...
  return Array.from(data)
    .map((item, index) => {
      return {
        ...
        name: item.name,
        number: item.number,
        status: (item.need_vertify === 3) ? '需要审核' : '未通过',
...
export default class LiteTable extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { tableData: [], next_page_url: null, visible: false };
    this.loadData();
  }

  loadData = () => {
    axios.get('https://igulu.net/api/getNeedVertifyStudent/', {
  ...
  loadMoreData = () => {
    axios.get('https://igulu.net/api/'.concat(this.state.next_page_url), {
      ...
      .then((res) => {
        ...
        newTableData.push(...generatorData(res.data.data));
  ...
  agree = (index) => {
    ...
    axios({
      method: 'POST',
      url: 'https://igulu.net/api/passStudent',
      data: bodyFormData,
  ...
  agreeAll = () => {
    ...
    axios.get('https://igulu.net/api/passAllStudent', {
      withCredentials: true,
    })
      .then((res) => {
        console.log(res);
  ...
  disagree = (index) => {
    ...
    axios({
      method: 'POST',
      url: 'https://igulu.net/api/denyStudent',
      data: bodyFormData,
      config: { headers: { 'Content-Type': 'multipart/form-data' } },
    })
      .then((res) => {
